...
agent_kit_marker_hash() {
  _agent_kit_path="$1"
  _agent_kit_marker="$2"
  if [ -f "$_agent_kit_marker" ]; then
    awk -v p="$_agent_kit_path" '$2 == p { print $1; exit }' "$_agent_kit_marker"
  fi
}

agent_kit_destination() {
  _agent_kit_rel="$1"
  case "$_agent_kit_rel" in
    .claude/*)
      printf '%s/%s' "$CLAUDE_CONFIG_DIR" "${_agent_kit_rel#.claude/}"
      ;;
    .codex/*)
      printf '%s/%s' "$CODEX_HOME" "${_agent_kit_rel#.codex/}"
      ;;
    *)
      printf '%s/%s' "$HOME" "$_agent_kit_rel"
      ;;
  esac
}

agent_kit_skip_home_copy() {
  case "$1" in
    .claude/.credentials.json|.codex/auth.json|.codex/config.toml|.claude/settings.json|.codex/hooks.json)
      return 0
      ;;
  esac
  return 1
...
agent_kit_copy_home_file() {
  _agent_kit_src="$1"
  _agent_kit_rel="$2"
  _agent_kit_dest="$(agent_kit_destination "$_agent_kit_rel")"
  _agent_kit_src_hash="$(sha256sum "$_agent_kit_src" | awk '{ print $1 }')"
  _agent_kit_old_hash="$(agent_kit_marker_hash "$_agent_kit_rel" "$_agent_kit_marker")"
  _agent_kit_record_hash=""

  mkdir -p "$(dirname "$_agent_kit_dest")"
  if [ ! -f "$_agent_kit_dest" ]; then
    cp -p "$_agent_kit_src" "$_agent_kit_dest"
    _agent_kit_record_hash="$_agent_kit_src_hash"
  elif [ -n "$_agent_kit_old_hash" ]; then
    _agent_kit_dest_hash="$(sha256sum "$_agent_kit_dest" | awk '{ print $1 }')"
    if [ "$_agent_kit_dest_hash" = "$_agent_kit_old_hash" ]; then
      if [ "$_agent_kit_dest_hash" != "$_agent_kit_src_hash" ]; then
        cp -p "$_agent_kit_src" "$_agent_kit_dest"
      fi
      _agent_kit_record_hash="$_agent_kit_src_hash"
    else
      if [ "$_agent_kit_old_hash" != "$_agent_kit_src_hash" ]; then
        echo "[entrypoint] WARN: ${_agent_kit_dest} has local changes and a stale agent-kit seed; leaving it unchanged"
      fi
      _agent_kit_record_hash="$_agent_kit_old_hash"
    fi
  else
    _agent_kit_dest_hash="$(sha256sum "$_agent_kit_dest" | awk '{ print $1 }')"
    if [ "$_agent_kit_dest_hash" = "$_agent_kit_src_hash" ]; then
  ...
  if [ -n "$_agent_kit_record_hash" ]; then
    printf '%s %s\n' "$_agent_kit_record_hash" "$_agent_kit_rel" >> "$_agent_kit_new_marker"
  fi
}

merge_agent_kit_hooks() {
  _agent_kit_source_json="$1"
  _agent_kit_dest_json="$2"
  [ -f "$_agent_kit_source_json" ] || return

  if ! command -v python3 >/dev/null 2>&1; then
    if [ ! -f "$_agent_kit_dest_json" ]; then
      mkdir -p "$(dirname "$_agent_kit_dest_json")"
      cp -p "$_agent_kit_source_json" "$_agent_kit_dest_json"
    fi
    return
  fi

  AK_HOOK_SOURCE="$_agent_kit_source_json" AK_HOOK_DEST="$_agent_kit_dest_json" python3 - <<'PY'
import json
import os
from pathlib import Path

source_path = Path(os.environ["AK_HOOK_SOURCE"])
dest_path = Path(os.environ["AK_HOOK_DEST"])
owned = {
...
def owns(group: object) -> bool:
    ...
    for hook in group.get("hooks", []):
        ...
        if any(name in command for name in owned):
            return True
    return False
...
for event, groups in source_hooks.items():
    if not isinstance(groups, list):
        continue
    existing = dest_hooks.get(event)
    if not isinstance(existing, list):
        existing = []
    desired = [group for group in groups if isinstance(group, dict)]
    retained = [group for group in existing if isinstance(group, dict) and not owns(group)]
    if desired or retained:
        dest_hooks[event] = desired + retained
    else:
        dest_hooks.pop(event, None)
...
write_agent_kit_version_markers() {
  [ -n "${AGENT_KIT_BAKED_VERSION:-}" ] || return
  mkdir -p "$CLAUDE_CONFIG_DIR" "$CODEX_HOME"
  printf 'version=%s\n' "$AGENT_KIT_BAKED_VERSION" > "${CLAUDE_CONFIG_DIR}/${AGENT_KIT_VERSION_MARKER}"
  printf 'version=%s\n' "$AGENT_KIT_BAKED_VERSION" > "${CODEX_HOME}/${AGENT_KIT_VERSION_MARKER}"
}

install_agent_kit_home() {
  [ -d "$AGENT_KIT_HOME_SOURCE" ] || return

  _agent_kit_marker="${HOME}/${AGENT_KIT_HOME_MARKER_FILE}"
  _agent_kit_new_marker="$(mktemp)"
  (
    cd "$AGENT_KIT_HOME_SOURCE"
    find . -type f | sort
  ) | while IFS= read -r _agent_kit_rel; do
    _agent_kit_rel="${_agent_kit_rel#./}"
    agent_kit_skip_home_copy "$_agent_kit_rel" && continue
    agent_kit_copy_home_file "${AGENT_KIT_HOME_SOURCE}/${_agent_kit_rel}" "$_agent_kit_rel"
  done

  if [ -s "$_agent_kit_new_marker" ]; then
    mkdir -p "$HOME"
    mv "$_agent_kit_new_marker" "$_agent_kit_marker"
  else
    rm -f "$_agent_kit_new_marker"
  fi

  merge_agent_kit_hooks "${AGENT_KIT_HOME_SOURCE}/.claude/settings.json" "${CLAUDE_CONFIG_DIR}/settings.json"
  merge_agent_kit_hooks "${AGENT_KIT_HOME_SOURCE}/.codex/hooks.json" "${CODEX_HOME}/hooks.json"
  write_agent_kit_version_markers
}
...
if [ ! -f "$HOME/.claude.json" ] && [ -d "$HOME/.claude/backups" ]; then
  latest=$(
    find "$HOME/.claude/backups" -maxdepth 1 -type f -name '.claude.json.backup.*' -printf '%T@ %p\n' 2>/dev/null \
      | sort -nr \
      | head -n1 \
      | cut -d' ' -f2- || true
  )
...
if [ ! -d "$AGENT_MCP_DIR" ] &&
   [ -n "${AGENT_KIT_MCP_FALLBACK_DIR:-}" ] &&
   [ -d "$AGENT_KIT_MCP_FALLBACK_DIR" ]; then
  AGENT_MCP_DIR="$AGENT_KIT_MCP_FALLBACK_DIR"
fi
