...
const CODEX_CREDENTIAL_POLL_MS = 100
const CODEX_CREDENTIAL_TIMEOUT_MS = 3_000
...
export class LoginSession {
  ...
  private async finalize(
    ...
    try {
      ...
      const bundle =
        this.provider === 'codex'
          ? await this.captureCodexWithRetry(updatedBy, reason)
          : await capture(this.provider, this.deps.paths, updatedBy, this.now, claudeToken)
  ...
  private async captureCodexWithRetry(updatedBy: string, reason: string) {
    const deadline = Date.now() + CODEX_CREDENTIAL_TIMEOUT_MS
    while (true) {
      try {
        return await capture(this.provider, this.deps.paths, updatedBy, this.now)
      } catch (err) {
        const missingCredential = (err as NodeJS.ErrnoException).code === 'ENOENT'
        if (!missingCredential || Date.now() >= deadline) {
          throw err
        }
        this.deps.logger.info('Codex credential files not ready; retrying capture', {
          sessionId: this.id,
          provider: this.provider,
          reason,
        })
        await sleep(CODEX_CREDENTIAL_POLL_MS)
      }
    }
  }
